...
// Set credit card as a default payment option and hide the rest
$('#payment option:contains("Credit Card")').prop('selected', true);
$('#paypal, #bitcoin').hide();

$('.activities').append('<div class="grandTotal"></div>');

// Displays or hides the job role input field
$('#other-title').hide();
$('#title').on('change', e => {
...
const errorMessage = (parent, type, value) => {
  ...
  //check credit card information when credit card is selected as payment method
  if (value.length) {
    if ($('#payment').val() === 'credit card') {
      if (
        ...
        outputError(
          `Please enter a ${fieldType} that is between 13 and 16 digits long.`
        );
      }
...
//change colors based on a selected theme
$('#design').on('change', e => {
  $('#color').empty();
...
const validateForm = () => {
  ...
  // Validates credit card information and checks the form
  if ($('#payment').val() === 'credit card') {
